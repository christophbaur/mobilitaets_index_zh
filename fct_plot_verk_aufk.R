#Visualisierung
fct_plot_verk_aufk <- function(data){
...
B_Axis <- list(side = "left", title = "Temperatur",
               ticksuffix="°C",
               range=c(-10, 29),
               dtick=10#,
               #zeroline = TRUE,
...
C_Axis <- list(side = "right", title = "Niederschlagsdauer", overlaying = "y2",
               ticksuffix=" Min.",
               range=c(0, 390),
               dtick=100#,
               #zeroline = TRUE,
...
fig2 <- plot_ly(mobility_index,
                x=~Date)%>%
  add_lines(y=~round(avg_7day_temp,0),
            mode='scatter',
            name='Temp.',
            line=list(color='#a5e7f0')
  )%>%
  add_bars(y=~round(avg_7day_rain,0),
           name='Nied.',
           yaxis='y2',
           color = I("#59c4e6"))%>%
  layout(#title='Wetter',
